...
ventana principal con el tamaño que tenía antes de activarse la vista mini,
en una posición válida: si la posición que tenía cae dentro de alguna
pantalla conectada, SHALL restaurarse ahí exactamente; si no, SHALL ubicarse
en una posición visible de la pantalla principal.

Entrar o salir de la vista mini SHALL NO alterar el estado de ningún timer.

#### Scenario: El usuario activa la vista mini

- **WHEN** el usuario acciona el control de vista mini en la ventana
  principal
...
- **AND** la ventana principal reaparece en la misma posición y tamaño que
  tenía al activarse la vista mini

#### Scenario: El usuario vuelve a la vista completa con doble click

- **WHEN** el usuario hace doble click sobre el fondo de la vista mini,
  fuera de cualquier control
...
- **WHEN** el usuario activó la vista mini con la ventana principal en una
  pantalla que se desconectó mientras la vista mini estaba en uso, y
  acciona el control de volver (o el doble click)
...
- **THEN** se presenta la ventana principal, nunca la vista mini, sin
  importar en qué vista estaba al cerrarla
