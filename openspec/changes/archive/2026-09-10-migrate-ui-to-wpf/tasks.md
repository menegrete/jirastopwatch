...
- [x] 6.1 Agregar `App.xaml` y `App.xaml.cs`, y mover a ahí el mutex de instancia única, los manejadores de excepción no atrapada y la suscripción a `SessionSwitch` de `Program.cs`
- [x] 6.2 Re-implementar la escucha de `WM_SHOWME` como hook sobre el handle de la ventana principal, en lugar de `WndProc` (ver `design.md` — D8)
- [x] 6.3 Crear el diccionario de recursos con los estilos base, apoyado en `ThemeBrushes`, que ya expone el tema a WPF
- [x] 6.4 Confirmar que `Theme` y `ThemeBrushes` cubren todos los colores que usaban los formularios; agregar los que falten

## 7. Lista de issues

- [x] 7.1 Crear la colección observable de modelos de issue como fuente de la lista
- [x] 7.2 Crear la plantilla de fila con key, summary, tiempo y los botones que tiene hoy `IssueControl`
- [x] 7.3 Hacer que el summary use hasta dos líneas y trunque al final de la segunda, con el key y el tiempo siempre visibles — spec `issue-list`, "El summary se acomoda al ancho disponible"
- [x] 7.4 Hacer que el alto de cada fila lo determine su contenido — spec `issue-list`, "Cada fila ocupa el alto que su contenido necesita"
- [x] 7.5 Crear la segunda plantilla de fila para la densidad compacta y el mecanismo que elige entre las dos (ver `design.md` — D6)
- [x] 7.6 Agregar el setting de densidad a `Settings`, exponerlo en el diálogo de settings y verificar que persiste — spec `issue-list`, "El usuario elige la densidad de la lista"
- [x] 7.7 Portar agregar, quitar, reordenar y seleccionar issue sobre la colección, incluido el límite de `MaxIssues`

## 8. Ventana principal

- [x] 8.1 Crear `MainWindow.xaml` con la barra superior, la lista y la barra inferior que tiene hoy `MainForm`
- [x] 8.2 Hacer el ancho ajustable con un mínimo que mantenga key y tiempo visibles, persistirlo, y validar el ancho guardado contra la pantalla disponible reutilizando `ScreenPlacement` — spec `issue-list`, "El usuario ajusta el ancho de la ventana principal"
- [x] 8.3 Derivar el alto de la suma de los altos de fila, sin permitir arrastrarlo, con el clamp al área de trabajo y el recorrido de la lista como desborde — spec `issue-list`, "El alto de la ventana se deriva de las filas" (ver `design.md` — D5)
- [x] 8.4 Portar los 13 atajos de teclado del inventario 1.1 como `InputBindings`, verificando uno por uno contra `ProcessCmdKey` antes de borrarlo
- [x] 8.5 Portar el ícono de bandeja usando `System.Windows.Forms.NotifyIcon`, con el disparador basado en el estado de la ventana en lugar de `MainForm_Resize`
- [x] 8.6 Portar el estado de conexión con Jira, la autenticación y el chequeo de actualizaciones
- [x] 8.7 Portar los tooltips, incluido el que cambia al llegar a `MaxIssues`
- [x] 8.8 Adaptar la vista mini para que su ventana principal sea la nueva `MainWindow`, conservando el guardado y la restauración de posición y tamaño

## 9. Diálogos

- [x] 9.1 `AboutWindow.xaml` — code-behind, misma información que `AboutForm`
- [x] 9.2 `EditTimeWindow.xaml` — code-behind, con la validación de tiempo que tiene hoy y su señalización de error
- [x] 9.3 `WorklogWindow.xaml` — code-behind, con las reglas de estimate y los tres resultados que hoy devuelve (`OK`, `Yes`, cancelar)
- [x] 9.4 `SettingsWindow.xaml` — code-behind, con todos los settings actuales más el de densidad
- [x] 9.5 Reemplazar `ModalDialog.ShowOver` por `Owner` + `Topmost` en los cuatro diálogos, y verificar que se ven por encima con "always on top" activado (ver `design.md` — D9)

## 10. Transiciones

- [x] 10.1 Animar el color del campo de tiempo al arrancar y al pausar un timer, terminando en un color del tema activo — spec `ui-theming`, "Los cambios de estado se presentan con una transición"
- [x] 10.2 Animar el cambio de fila seleccionada, con la animación nueva reemplazando a la anterior en vez de encolarse (ver `design.md` — D7)
- [x] 10.3 Resolver el color de llegada contra el tema vigente al terminar la animación, para el caso de cambio de tema en curso
- [x] 10.4 Verificar que el estado sigue siendo distinguible durante la animación y no sólo al final

## 11. Limpieza

- [x] 11.1 Borrar `UI/MainForm.cs`, `UI/MainForm.Designer.cs`, `UI/IssueControl.cs`, `UI/SettingsForm.*`, `UI/WorklogForm.*`, `UI/EditTimeForm.*`, `UI/AboutForm.*`
- [x] 11.2 Borrar `UI/ThemeApplier.cs` y `UI/ModalDialog.cs`
- [x] 11.3 Borrar de `Helpers/NativeMethods.cs` las llamadas de modo oscuro (`UseImmersiveDarkMode`, `UseDarkScrollBars`, `UseDarkComboBox`, `AllowDarkModeForApp` y el `SetPreferredAppMode` por ordinal), conservando `WM_SHOWME` y lo que siga en uso
- [x] 11.4 Borrar `Helpers/InvokeExtensions.cs` y `UI/ComboTextBoxEvents.cs` si ya no los usa nadie
- [x] 11.5 Revisar `Program.cs`: dejarlo sólo si algo sigue necesitándolo, o borrarlo si `App.xaml.cs` lo reemplazó por completo
- [x] 11.6 Revisar `UI/BoolConverters.cs` y `UI/ThemeIcons.cs`: adaptar o borrar según sigan aplicando
- [x] 11.7 Confirmar que `UseWindowsForms` sigue en `true` en el `.csproj`, con un comentario que explique que es por la bandeja y la enumeración de pantallas
